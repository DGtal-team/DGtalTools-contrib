...
class RationalBeattySequence
{
  ...
  public:
     ...
     * ∀(n,m) ∈ ℕ², this(n) < m or inverse(n) < m but not both \cite
     * lambek1954amm.
     *
    ...
    RationalBeattySequence complement() const
    {
        ...
        boost::rational<int> r(
            _ratio.numerator(), _ratio.numerator() - _ratio.denominator());
        return RationalBeattySequence(r, -_offset - 1);
    }
    ...
    friend std::ostream &operator<<(
        std::ostream &out, const RationalBeattySequence &seq)
    {
        out << "⌊";
        if (seq._offset != 0)
            out << '(';
    ...
    {
        return _ratio == otherSeq._ratio && _offset == otherSeq._offset;
    }
};
